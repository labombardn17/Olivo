    ...
    <section id="treatments" aria-labelledby="concerns-title" className="gap-section !pt-0">
      <div className="frame grid md:grid-cols-2">
        ...
        <div className="px-6 py-12 md:px-14 md:py-20 flex flex-col justify-center">
          ...
          <ul role="tablist" aria-label="Concerns" className="mt-8 grid grid-cols-2 gap-x-8">
            {concerns.map((x, k) => (
              <li key={x.key} className="border-b border-rule">
                <button type="button" role="tab" id={`concern-${x.key}`} aria-selected={k === i} aria-controls="concern-panel" onClick={() => setI(k)} onFocus={() => setI(k)} className={`block w-full text-left py-3 font-display text-[1.35rem] transition-colors duration-500 ${k === i ? "text-ink" : "text-ink-2"}`}>
                  {x.label}
                </button>
              </li>
            ))}
