      ...
      <ol className="flex flex-wrap items-center gap-1.5">
        {items.map((it, i) => (
          <li key={i} className="inline-flex items-center gap-1.5">
            {it.href ? <Link href={it.href} className="hover:underline underline-offset-4">{it.name}</Link> : <span aria-current="page" className="text-ink">{it.name}</span>}
...
export function FaqList({ faqs, title = "Good to know", id = "faq" }: { faqs: Faq[]; title?: string; id?: string }) {
  if (!faqs.length) return null;
  return (
    <section aria-labelledby={`${id}-title`} className="py-20 md:py-24">
      <div className="container-x mx-auto max-w-3xl">
        <h2 id={`${id}-title`} className="section-title text-center">{title}</h2>
...
export function CtaBand({ title = "Ready when you are.", line = "Book online any time, text us, or call the clinic during hours.", sms }: { title?: string; line?: string; sms?: string }) {
  return (
      ...
      <div className="container-x">
        <Reveal className="mx-auto max-w-3xl text-center">
          ...
          <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
            <Magnetic><a href={site.booking} data-cta="primary" className="btn btn-primary">Book a consultation</a></Magnetic>
          ...
          <p className="mt-5 text-[0.875rem] text-ink-2">Financing available through Cherry. Terms are set by Cherry at application.<Verify note={site.financingVerify} /></p>
        </Reveal>
      </div>
    </section>
  );
...
export function Steps({ steps, title = "What to expect" }: { steps: { step: string; line: string }[]; title?: string }) {
  return (
    <div>
...
export function LinkCard({ href, title, line, meta, className = "" }: { href: string; title: string; line: string; meta?: string; className?: string }) {
  return (
    <Link href={href} className={`card card-link lift group flex h-full flex-col p-6 ${className}`}>
      ...
      <span className="link-arrow mt-4 text-[0.875rem]">Learn more <Arrow /></span>
    </Link>
  );
}

export function Notice({ children }: { children: ReactNode }) {
  return <p className="text-[0.8125rem] leading-relaxed text-ink-2">{children}</p>;
}
