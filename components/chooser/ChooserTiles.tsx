      ...
      {concepts.map((c, i) => {
        ...
        return (
          <li key={c.key} className={wide ? "md:col-span-4" : i === 4 ? "md:col-span-6 md:grid md:grid-cols-6 md:gap-8" : "md:col-span-2"}>
            <Link prefetch={false} href={`/${c.key}${chosen ? `?palette=${palette}` : ""}`} className={`group block ${i === 4 ? "md:col-span-4" : ""}`}>
              ...
              <div className="mt-4 flex items-baseline gap-4">
                ...
                <h2 className="font-display text-[1.75rem] leading-none" style={{ fontVariationSettings: '"opsz" 72' }}>{c.name}</h2>
            ...
            {i === 4 && (
              <div className="md:col-span-2 mt-6 md:mt-0 text-[13px] text-ink-2 leading-relaxed self-end">
                <p>Also: <Link prefetch={false} href="/compare" className="u-draw text-ink">compare two side by side</Link>.</p>
